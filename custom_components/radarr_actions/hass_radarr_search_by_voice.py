...
class MovieDownloader:
    ...
    def __init__(self, host, api_key, root_directory="", profile_id="", tmdbid_api_key_v3=""):
        ...
        # self.loadParameters(configFileName)
        ...
        # self.search(movie, mode, configFileName, voice_feedback)



    def search(self, movie, mode=0, range_of_years=-1, search=False):
        ...
        if (range_of_years != -1):
            for i  in range(1,range_of_years):
                ...
                if r.status_code == requests.codes.ok:
                    ...
                    if len(media_list) > 0:
                        ...
                        if mode == 0: # download best guess
                            ...
                            found = False
                            if (range_of_years == -1): #no year restrictions
                                logging.debug("Mode 0: Automatically adding first match.")
                                found = True
                                logging.debug("movie "+media_list[0]['title']+" found")
                                data = self.prepare_movie_json(media_list[0], search)
                                self.add_movie(data)
                            else:
                                i = 0
                                found = False
                                while i < len(media_list) and found == False:
                                    ...
                                    if year in current_years:
                                        ...
                                        data = self.prepare_movie_json(media_list[i], search)
                                        self.add_movie(data)
                                        break;
                                    i += 1
                            if found == False:
                        ...
                        msg = "Your radarr setup seems fine, but it didn't found a result."
                        self.tts_google(msg)
                        logging.debug(msg)

                else:
                    self.status = 'failed'
                    msg = "Radarr didn't respond. Please check your host and api_key for Radarr."
                    self.tts_google(msg)
                    logging.debug(msg)
    ...
    def remove_movie(self, id):
        ...
        r = requests.delete(self.RADARR_SERVER+self.baseUrl+"/"+str(id), headers=self.headers)
        logging.debug("Trying to delete movie...")
        ...
        if r.status_code == 201:
            ...
            if res >= 0:
                if res == 0:
                    ...
                    self.code = self.MOVIE_EXISTS_OK
                    if str(data['cast']) == "":
    ...
    # def loadParameters(self, configFileName='ha_radarr_sonarr.conf'):
    #     config=configparser.ConfigParser()
    #     dirname, filename = os.path.split(os.path.abspath(sys.argv[0]))
    #     configFile = os.path.join(dirname, configFileName)
...
if __name__ == "__main__":
    ...
    parser.add_argument("mode", help="Running mode. 0=Add best match. 1=Full search. 2=Add numeric option. 3=Search by actor",
                        type=str)
    parser.add_argument("configFile", nargs='?', default="ha_radarr_sonarr.conf", help="Define the name of a config file. Useful for having more than 1 setup",
                        type=str)
    parser.add_argument("voice_feedback", nargs='?', default="False", help="Use a smart speaker for the output",
                        type=str)
    parser.add_argument(
        '-d', '--debug',
    ...
    use_voice = False
    if args.voice_feedback == "True":
        use_voice = True
    elif args.voice_feedback =="False":
        use_voice = False

    downloader = MovieDownloader(args.query, int(args.mode), args.configFile, use_voice)
    downloader.search(args.query, int(args.mode), args.configFile, use_voice)
